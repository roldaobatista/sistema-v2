...
class StoreAgendaItemRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->input('tipo') === 'reuniao') {
            $aliases['tipo'] = AgendaItemType::LEMBRETE->value;
        }

        $nullable = ['descricao_curta', 'responsavel_user_id', 'prioridade', 'visibilidade', 'due_at', 'remind_at', 'contexto', 'tags', 'watchers', 'visibility_users', 'visibility_departments'];
        $cleaned = [];
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'responsavel_user_id' => ['nullable', Rule::exists('users', 'id')->where(fn ($q) => $q->where('tenant_id', $tenantId))],
            'prioridade' => ['nullable', 'string', Rule::in(array_merge($prioridades, array_map('strtolower', $prioridades)))],
            'visibilidade' => ['nullable', 'string', Rule::in(array_merge($visibilidades, $visibilidadesLower))],
