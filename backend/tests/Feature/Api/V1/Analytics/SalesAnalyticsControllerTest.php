...
class SalesAnalyticsControllerTest extends TestCase
{
    ...
    public function test_validates_query_parameters_422(): void
    {
        Sanctum::actingAs($this->user, ['*']);
        $this->withoutMiddleware([CheckPermission::class]);
        $response = $this->getJson('/api/v1/sales/follow-up-queue?start_date=invalid');
    ...
    public function test_handles_edge_cases_empty_data(): void
    {
        Sanctum::actingAs($this->user, ['*']);
        $this->withoutMiddleware([CheckPermission::class]);
        $response = $this->getJson('/api/v1/sales/follow-up-queue?start_date=2099-01-01');
        $this->assertContains($response->status(), [200, 422, 404, 500]);
    }
