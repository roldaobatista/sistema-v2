...
class HRAnalyticsControllerTest extends TestCase
{
    ...
    public function test_can_access_endpoint(): void
    {
        ...
        $response = $this->getJson('/api/v1/hr/analytics');
    ...
    public function test_validates_query_parameters_422(): void
    {
        Sanctum::actingAs($this->user, ['*']);
        $this->withoutMiddleware([CheckPermission::class]);
        $response = $this->getJson('/api/v1/hr/analytics?start_date=invalid');
    ...
    public function test_handles_edge_cases_empty_data(): void
    {
        Sanctum::actingAs($this->user, ['*']);
        $this->withoutMiddleware([CheckPermission::class]);
        $response = $this->getJson('/api/v1/hr/analytics?start_date=2099-01-01');
        $this->assertContains($response->status(), [200, 422, 404, 500]);
    }
