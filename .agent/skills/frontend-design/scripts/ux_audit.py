...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        has_real_form = bool(re.search(r'<form', content, re.IGNORECASE))
        input_count = len(re.findall(r'<input(?!\s+type=["\']hidden["\'])|<select|<textarea', content, re.IGNORECASE))
        has_form = has_real_form or input_count > 0
        complex_elements = input_count + len(re.findall(r'<option', content, re.IGNORECASE))
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {
                ...
                'storage', '.worktrees', '.superpowers', '.agent', '.codex',
                '.cursor', '.zap', 'playwright-report', 'e2e'
            }]
            for file in files:
