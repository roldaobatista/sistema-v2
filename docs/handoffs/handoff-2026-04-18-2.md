...
### Opção B — Ajustar Dockerfile para bind mount do projeto
Editar `docker-compose.test.yml` para montar raiz do projeto em `/var/` (solução de médio prazo):
```yaml
volumes:
  - .:/workspace:ro
  - .:/var/host-project:ro  # permitiria que os 20 testes achem CLAUDE.md/nginx/.github
```

Não fiz essa mudança pois envolve ajustes nos paths dos testes Unit.

### Opção C — Confiar no resultado Docker atual
Considerando que:
...
- **Nenhum commit novo** — esta sessão foi investigativa/validação, não alterou código nem docs.

## Estado ao sair

- **Working tree limpo.** 0 commits novos.
...
Após confirmar suite verde local, rodar:
```
/reaudit "Camada 1"
```
...
Alternativamente, aceitar o resultado Docker como suficiente (justificativa sólida: os 20 falhas são ambiente-only e totalizam **9762** testes iguais ao último verde).
