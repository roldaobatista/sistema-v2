import React, { useState } from 'react'
import { useForm, Controller } from 'react-hook-form'
import type { Resolver } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import type { AxiosError } from 'axios'
import { broadcastQueryInvalidation } from '@/lib/cross-tab-sync'
import { useDebounce } from '@/hooks/useDebounce'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import { Search, Plus, Pencil, Trash2, Package, AlertTriangle, UploadCloud, Scale, ChevronLeft, ChevronRight, MapPin, Hash } from 'lucide-react'
import { toast } from 'sonner'
import api, { getApiErrorMessage } from '@/lib/api'
import { useAuvoExport } from '@/hooks/useAuvoExport'
import { cn, formatCurrency } from '@/lib/utils'
import { Button } from '@/components/ui/button'
import { IconButton } from '@/components/ui/iconbutton'
import { Input } from '@/components/ui/input'
import { Badge } from '@/components/ui/badge'
import { Modal } from '@/components/ui/modal'
import { FormField } from '@/components/ui/form-field'
import { PageHeader } from '@/components/ui/pageheader'
import { EmptyState } from '@/components/ui/emptystate'
import { useAuthStore } from '@/stores/auth-store'
import { CurrencyInput } from '@/components/common/CurrencyInput'
import { handleFormError } from '@/lib/form-utils'
import { optionalString, requiredString } from '@/schemas/common'
import { extractDeleteConflict } from '@/types/api'
import { z } from 'zod'

interface Product {
    id: number; code: string | null; name: string; description: string | null
    unit: string; cost_price: string; sell_price: string
    stock_qty: string; stock_min: string; is_active: boolean
    track_stock: boolean; track_batch: boolean; track_serial: boolean; is_kit: boolean
    min_repo_point: string | null; max_stock: string | null
    manufacturer_code: string | null; storage_location: string | null
    ncm: string | null; default_supplier_id: number | null
    image_url: string | null; barcode: string | null; brand: string | null
    weight: string | null; width: string | null; height: string | null; depth: string | null
    category: { id: number; name: string } | null
    default_supplier?: { id: number; name: string } | null
    equipment_models?: { id: number; name: string; brand: string | null; category: string | null }[]
}

const productSchema = z.object({
    category_id: z.union([z.string(), z.number()]).optional().default(''),
    default_supplier_id: z.union([z.string(), z.number()]).optional().default(''),
    code: optionalString,
    name: requiredString('Nome é obrigatório'),
    description: optionalString,
    unit: z.string().default('UN'),
    cost_price: z.string().default('0'),
    sell_price: z.string().default('0'),
    stock_qty: z.string().default('0'),
    stock_min: z.string().default('0'),
    min_repo_point: z.string().default('0'),
    max_stock: z.string().default('0'),
    is_active: z.boolean().default(true),
    track_stock: z.boolean().default(true),
    track_batch: z.boolean().default(false),
    track_serial: z.boolean().default(false),
    is_kit: z.boolean().default(false),
    manufacturer_code: optionalString,
    storage_location: optionalString,
    ncm: optionalString,
    image_url: optionalString,
    barcode: optionalString,
    brand: optionalString,
    weight: z.string().default(''),
    width: z.string().default(''),
    height: z.string().default(''),
    depth: z.string().default(''),
})

type ProductFormData = z.infer<typeof productSchema>

const defaultValues: ProductFormData = {
    category_id: '', default_supplier_id: '', code: '', name: '', description: '',
    unit: 'UN', cost_price: '0', sell_price: '0',
    stock_qty: '0', stock_min: '0', min_repo_point: '0', max_stock: '0',
    is_active: true, track_stock: true, track_batch: false, track_serial: false, is_kit: false,
    manufacturer_code: '', storage_location: '', ncm: '',
    image_url: '', barcode: '', brand: '',
    weight: '', width: '', height: '', depth: '',
}

export function ProductsPage() {
    const { hasPermission } = useAuthStore()

    const canCreate = hasPermission('cadastros.product.create')
    const canEdit = hasPermission('cadastros.product.update')
    const canDelete = hasPermission('cadastros.product.delete')

    const qc = useQueryClient()
    const { exportProduct } = useAuvoExport()
    const [search, setSearch] = useState('')
    const debouncedSearch = useDebounce(search, 400)
    const [page, setPage] = useState(1)
    const [showForm, setShowForm] = useState(false)
    const [editing, setEditing] = useState<Product | null>(null)
    const [newCat, setNewCat] = useState('')

    const [showConfirmDelete, setShowConfirmDelete] = useState<Product | null>(null)
    const [deleteDependencies, setDeleteDependencies] = useState<Record<string, number> | null>(null)
    const [deleteMessage, setDeleteMessage] = useState<string | null>(null)

    const { register, handleSubmit, reset, control, setError, formState: { errors } } = useForm<ProductFormData>({
        resolver: zodResolver(productSchema) as Resolver<ProductFormData>,
        defaultValues,
    })

    const perPage = 25

    const { data: res, isLoading } = useQuery({
        queryKey: ['products', debouncedSearch, page],
        queryFn: () => api.get('/products', { params: { search: debouncedSearch, per_page: perPage, page } }),
    })
    const products: Product[] = res?.data?.data ?? []
    const totalProducts: number = res?.data?.meta?.total ?? res?.data?.total ?? 0
    const lastPage: number = res?.data?.meta?.last_page ?? res?.data?.last_page ?? 1

    const { data: catsRes } = useQuery({
        queryKey: ['product-categories'],
        queryFn: () => api.get('/product-categories'),
    })
    const categories = catsRes?.data?.data ?? catsRes?.data ?? []

    const { data: suppRes } = useQuery({
        queryKey: ['suppliers-select'],
        queryFn: () => api.get('/suppliers', { params: { per_page: 9999, is_active: true } }),
    })
    const suppliers: { id: number; name: string }[] = suppRes?.data?.data ?? []

    const saveMut = useMutation({
        mutationFn: (data: ProductFormData) =>
            editing ? api.put(`/products/${editing.id}`, data) : api.post('/products', data),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['products'] })
            broadcastQueryInvalidation(['products', 'products-all', 'stock'], 'Produto')
            setShowForm(false)
            toast.success(editing ? 'Produto atualizado com sucesso!' : 'Produto criado com sucesso!')
        },
        onError: (err: unknown) => handleFormError(err as AxiosError<{ message: string; errors?: Record<string, string[]> }>, setError, 'Erro ao salvar produto.'),
    })

    const deleteMut = useMutation({
        mutationFn: (id: number) => api.delete(`/products/${id}`),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['products'] })
            qc.invalidateQueries({ queryKey: ['stock'] })
            broadcastQueryInvalidation(['products', 'products-all', 'stock'], 'Produto')
            setShowConfirmDelete(null)
            toast.success('Produto excluído com sucesso!')
        },
        onError: (err: unknown) => {
            const deleteConflict = extractDeleteConflict(err)

            if (deleteConflict) {
                setDeleteDependencies(deleteConflict.dependencies)
                setDeleteMessage(deleteConflict.message)
                return
            }

            toast.error(getApiErrorMessage(err, 'Erro ao excluir produto.'))
            setShowConfirmDelete(null)
        },
    })

    const catMut = useMutation({
        mutationFn: (name: string) => api.post('/product-categories', { name }),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['product-categories'] })
            setNewCat('')
            toast.success('Categoria criada!')
        },
        onError: (err: unknown) => toast.error(getApiErrorMessage(err, 'Erro ao criar categoria.'))
    })

    const { data: productDetail } = useQuery({
        queryKey: ['product', editing?.id],
        queryFn: () => api.get(`/products/${editing!.id}`).then((r) => r.data?.data ?? r.data),
        enabled: !!editing?.id && showForm,
    })
    const productEquipmentModels: { id: number; name: string; brand: string | null }[] =
        productDetail?.equipment_models ?? productDetail?.equipmentModels ?? []

    const openCreate = () => { setEditing(null); reset(defaultValues); setShowForm(true) }
    const openEdit = (p: Product) => {
        setEditing(p)
        reset({
            category_id: p.category?.id ?? '', code: p.code ?? '', name: p.name,
            description: p.description ?? '', unit: p.unit,
            cost_price: p.cost_price, sell_price: p.sell_price,
            stock_qty: p.stock_qty, stock_min: p.stock_min, is_active: p.is_active,
            track_stock: p.track_stock ?? true, track_batch: p.track_batch ?? false,
            track_serial: p.track_serial ?? false, is_kit: p.is_kit ?? false,
            min_repo_point: p.min_repo_point ?? '0', max_stock: p.max_stock ?? '0',
            default_supplier_id: p.default_supplier_id ?? '',
            manufacturer_code: p.manufacturer_code ?? '', storage_location: p.storage_location ?? '',
            ncm: p.ncm ?? '',
            image_url: p.image_url ?? '', barcode: p.barcode ?? '', brand: p.brand ?? '',
            weight: p.weight ?? '', width: p.width ?? '', height: p.height ?? '', depth: p.depth ?? '',
        })
        setShowForm(true)
    }

    const handleSearch = (val: string) => {
        setSearch(val)
        setPage(1)
    }

    const isLowStock = (p: Product) => {
        const min = parseFloat(p.stock_min)
        return min > 0 && parseFloat(p.stock_qty) <= min
    }

    if (isLoading) return (
        <div className="space-y-5 animate-fade-in">
            <div className="flex items-center justify-between">
                <div>
                    <div className="skeleton h-7 w-32" />
                    <div className="skeleton mt-2 h-4 w-48" />
                </div>
                <div className="skeleton h-9 w-28" />
            </div>
            <div className="skeleton h-10 w-full max-w-md" />
            <div className="rounded-xl border border-default bg-surface-0 h-96"></div>
        </div>
    )

    return (
        <div className="space-y-5 animate-fade-in">
            <PageHeader
                title="Produtos"
                subtitle="Catálogo de produtos e peças"
                count={totalProducts}
                actions={canCreate ? [{ label: 'Novo Produto', onClick: openCreate, icon: <Plus className="h-4 w-4" /> }] : []}
            />

            <div className="max-w-sm">
                <div className="relative">
                    <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-surface-400" />
                    <input type="text" value={search} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleSearch(e.target.value)}
                        placeholder="Buscar por nome ou código..."
                        className="w-full rounded-lg border border-default bg-surface-50 py-2.5 pl-10 pr-4 text-sm focus:border-brand-400 focus:bg-surface-0 focus:outline-none focus:ring-2 focus:ring-brand-500/15" />
                </div>
            </div>

            <div className="overflow-hidden rounded-xl border border-default bg-surface-0 shadow-card">
                <table className="w-full">
                    <thead>
                        <tr className="border-b border-subtle bg-surface-50">
                            <th className="px-3.5 py-2.5 text-left text-xs font-medium uppercase tracking-wider text-surface-500">Produto</th>
                            <th className="hidden px-3.5 py-2.5 text-left text-xs font-medium uppercase tracking-wider text-surface-500 md:table-cell">Categoria</th>
                            <th className="hidden px-3.5 py-2.5 text-left text-xs font-medium uppercase tracking-wider text-surface-500 lg:table-cell">Marca</th>
                            <th className="hidden px-3.5 py-2.5 text-left text-xs font-medium uppercase tracking-wider text-surface-500 xl:table-cell">Cód. Fab.</th>
                            <th className="hidden px-3.5 py-2.5 text-left text-xs font-medium uppercase tracking-wider text-surface-500 xl:table-cell">Local</th>
                            <th className="px-3.5 py-2.5 text-right text-xs font-medium uppercase tracking-wider text-surface-500">Preço</th>
                            <th className="hidden px-3.5 py-2.5 text-right text-xs font-medium uppercase tracking-wider text-surface-500 lg:table-cell">Estoque</th>
                            <th className="px-3.5 py-2.5 text-right text-xs font-medium uppercase tracking-wider text-surface-500">Ações</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-subtle">
                        {products.length === 0 ? (
                            <tr><td colSpan={8} className="px-4 py-2">
                                <EmptyState
                                    icon={<Package className="h-5 w-5 text-surface-300" />}
                                    message="Nenhum produto encontrado"
                                    action={canCreate ? { label: 'Novo Produto', onClick: openCreate, icon: <Plus className="h-4 w-4" /> } : undefined}
                                    compact
                                />
                            </td></tr>
                        ) : (products || []).map(p => (
                            <tr key={p.id} className="hover:bg-surface-50 transition-colors duration-100">
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-3">
                                        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-50">
                                            <Package className="h-4 w-4 text-brand-600" />
                                        </div>
                                        <div>
                                            <p className="text-sm font-medium text-surface-900">{p.name}</p>
                                            {p.code && <p className="text-xs text-surface-400">#{p.code}</p>}
                                        </div>
                                    </div>
                                </td>
                                <td className="hidden px-4 py-3 md:table-cell">
                                    {p.category ? <Badge variant="brand">{p.category.name}</Badge> : <span className="text-xs text-surface-400">—</span>}
                                </td>
                                <td className="hidden px-3.5 py-2.5 text-sm text-surface-600 lg:table-cell">
                                    {p.brand ? (
                                        <span className="text-sm font-medium text-surface-700">{p.brand}</span>
                                    ) : <span className="text-xs text-surface-400">—</span>}
                                </td>
                                <td className="hidden px-3.5 py-2.5 text-sm text-surface-600 xl:table-cell">
                                    {p.manufacturer_code ? (
                                        <span className="flex items-center gap-1"><Hash className="h-3 w-3 text-surface-400" />{p.manufacturer_code}</span>
                                    ) : <span className="text-xs text-surface-400">—</span>}
                                </td>
                                <td className="hidden px-3.5 py-2.5 text-sm text-surface-600 xl:table-cell">
                                    {p.storage_location ? (
                                        <span className="flex items-center gap-1"><MapPin className="h-3 w-3 text-surface-400" />{p.storage_location}</span>
                                    ) : <span className="text-xs text-surface-400">—</span>}
                                </td>
                                <td className="px-3.5 py-2.5 text-right text-sm font-medium text-surface-900">{formatCurrency(parseFloat(p.sell_price))}</td>
                                <td className="hidden px-3.5 py-2.5 text-right lg:table-cell">
                                    <div className="flex items-center justify-end gap-1.5">
                                        {isLowStock(p) && <AlertTriangle className="h-3.5 w-3.5 text-amber-500" />}
                                        <span className={cn('text-sm font-medium', isLowStock(p) ? 'text-amber-600' : 'text-surface-700')}>
                                            {parseFloat(p.stock_qty)} {p.unit}
                                        </span>
                                    </div>
                                </td>
                                <td className="px-4 py-3">
                                    <div className="flex items-center justify-end gap-1">
                                        {hasPermission('auvo.export.execute') && (
                                            <IconButton
                                                icon={<UploadCloud className="h-4 w-4" />}
                                                aria-label="Exportar para Auvo"
                                                tooltip="Exportar para Auvo"
                                                onClick={() => exportProduct.mutate(p.id)}
                                                className="hover:text-blue-600 hover:bg-blue-50"
                                                disabled={exportProduct.isPending}
                                            />
                                        )}
                                        {canEdit && (
                                            <IconButton label="Editar" icon={<Pencil className="h-4 w-4" />} onClick={() => openEdit(p)} className="hover:text-brand-600" />
                                        )}
                                        {canDelete && (
                                            <IconButton label="Excluir" icon={<Trash2 className="h-4 w-4" />} onClick={() => {
                                                setShowConfirmDelete(p)
                                                setDeleteDependencies(null)
                                                setDeleteMessage(null)
                                            }} className="hover:text-red-600" />
                                        )}
                                    </div>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            </div>

            {lastPage > 1 && (
                <div className="flex items-center justify-between">
                    <p className="text-sm text-surface-500">
                        Mostrando {(page - 1) * perPage + 1}–{Math.min(page * perPage, totalProducts)} de {totalProducts}
                    </p>
                    <div className="flex items-center gap-1">
                        <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage(p => p - 1)}>
                            <ChevronLeft className="h-4 w-4" />
                        </Button>
                        <span className="px-3 text-sm text-surface-700">Página {page} de {lastPage}</span>
                        <Button variant="outline" size="sm" disabled={page >= lastPage} onClick={() => setPage(p => p + 1)}>
                            <ChevronRight className="h-4 w-4" />
                        </Button>
                    </div>
                </div>
            )}

            <Modal open={showForm} onOpenChange={setShowForm} title={editing ? 'Editar Produto' : 'Novo Produto'} size="lg">
                <form onSubmit={handleSubmit((data: ProductFormData) => saveMut.mutate(data))} className="space-y-5">

                    {/* ── Dados Básicos ── */}
                    <fieldset className="space-y-4">
                        <legend className="text-sm font-semibold text-surface-700 uppercase tracking-wide mb-2">Dados Básicos</legend>
                        <div className="grid gap-4 sm:grid-cols-2">
                            <FormField label="Nome" error={errors.name?.message} required>
                                <Input {...register('name')} />
                            </FormField>
                            <FormField label="Código" error={errors.code?.message}>
                                <Input {...register('code')} placeholder="Opcional" />
                            </FormField>
                            <FormField label="Código fabricante" error={errors.manufacturer_code?.message}>
                                <Input {...register('manufacturer_code')} placeholder="Opcional" />
                            </FormField>
                            <FormField label="Unidade" error={errors.unit?.message}>
                                <Input {...register('unit')} placeholder="UN, CX, KG..." />
                            </FormField>
                            <FormField label="Marca" error={errors.brand?.message}>
                                <Input {...register('brand')} placeholder="Ex: Bosch, 3M..." />
                            </FormField>
                        </div>
                        <div className="grid gap-4 sm:grid-cols-3">
                            <div>
                                <FormField label="Categoria" error={errors.category_id?.message}>
                                    <select id="product-category" {...register('category_id')} aria-label="Categoria do produto"
                                        className="w-full rounded-lg border border-default bg-surface-50 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:bg-surface-0 focus:outline-none focus:ring-2 focus:ring-brand-500/15">
                                        <option value="">Sem categoria</option>
                                        {(categories || []).map((c: { id: number; name: string }) => <option key={c.id} value={c.id}>{c.name}</option>)}
                                    </select>
                                </FormField>
                                <div className="mt-1.5 flex gap-1">
                                    <input value={newCat} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewCat(e.target.value)} placeholder="Nova categoria"
                                        className="flex-1 rounded-md border border-default px-2 py-1 text-xs" />
                                    <Button variant="ghost" size="sm" type="button" disabled={!newCat}
                                        onClick={() => catMut.mutate(newCat)}>+</Button>
                                </div>
                            </div>
                            <FormField label="Fornecedor padrão" error={errors.default_supplier_id?.message}>
                                <select id="product-supplier" {...register('default_supplier_id')} aria-label="Fornecedor padrão"
                                    className="w-full rounded-lg border border-default bg-surface-50 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:bg-surface-0 focus:outline-none focus:ring-2 focus:ring-brand-500/15">
                                    <option value="">Nenhum</option>
                                    {(suppliers || []).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                                </select>
                            </FormField>
                            <div />
                        </div>
                        <FormField label="Descrição" error={errors.description?.message}>
                            <textarea id="product-description" {...register('description')} placeholder="Descrição do produto" rows={2}
                                className="w-full rounded-lg border border-default bg-surface-50 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:bg-surface-0 focus:outline-none focus:ring-2 focus:ring-brand-500/15" />
                        </FormField>
                    </fieldset>

                    <hr className="border-subtle" />

                    {/* ── Preços ── */}
                    <fieldset className="space-y-4">
                        <legend className="text-sm font-semibold text-surface-700 uppercase tracking-wide mb-2">Preços</legend>
                        <div className="grid gap-4 sm:grid-cols-2">
                            <Controller control={control} name="cost_price" render={({ field }) => (
                                <FormField label="Preço Custo">
                                    <CurrencyInput label="" value={parseFloat(field.value) || 0} onChange={val => field.onChange(String(val))} />
                                </FormField>
                            )} />
                            <Controller control={control} name="sell_price" render={({ field }) => (
                                <FormField label="Preço Venda">
                                    <CurrencyInput label="" value={parseFloat(field.value) || 0} onChange={val => field.onChange(String(val))} />
                                </FormField>
                            )} />
                        </div>
                    </fieldset>

                    <hr className="border-subtle" />

                    {/* ── Estoque ── */}
                    <fieldset className="space-y-4">
                        <legend className="text-sm font-semibold text-surface-700 uppercase tracking-wide mb-2">Estoque</legend>
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            <FormField label={editing ? 'Estoque atual (via movimentações)' : 'Estoque inicial'} error={errors.stock_qty?.message}>
                                <Input {...register('stock_qty')} type="number" step="0.01" disabled={!!editing}
                                    title={editing ? 'Estoque é alterado via movimentações' : undefined} />
                            </FormField>
                            <FormField label="Estoque Mín." error={errors.stock_min?.message}>
                                <Input {...register('stock_min')} type="number" step="0.01" />
                            </FormField>
                            <FormField label="Ponto Reposição" error={errors.min_repo_point?.message}>
                                <Input {...register('min_repo_point')} type="number" step="0.01" placeholder="0" />
                            </FormField>
                            <FormField label="Estoque Máx." error={errors.max_stock?.message}>
                                <Input {...register('max_stock')} type="number" step="0.01" placeholder="0" />
                            </FormField>
                        </div>
                        <FormField label="Endereço no estoque" error={errors.storage_location?.message}>
                            <Input {...register('storage_location')} placeholder="Ex: A-01-02" />
                        </FormField>
                    </fieldset>

                    <hr className="border-subtle" />

                    {/* ── Controles Avançados ── */}
                    <fieldset className="space-y-3">
                        <legend className="text-sm font-semibold text-surface-700 uppercase tracking-wide mb-2">Controles Avançados</legend>
                        <div className="grid gap-3 sm:grid-cols-2">
                            <Controller control={control} name="track_stock" render={({ field }) => (
                                <label className="flex items-center gap-2 cursor-pointer rounded-lg border border-default p-3 hover:bg-surface-50 transition-colors">
                                    <input type="checkbox" checked={field.value} onChange={e => field.onChange(e.target.checked)}
                                        className="h-4 w-4 rounded border-default text-brand-600 focus:ring-brand-500" />
                                    <div>
                                        <span className="text-sm font-medium text-surface-700">Controlar estoque</span>
                                        <p className="text-xs text-surface-400">Habilita entrada/saída e saldo</p>
                                    </div>
                                </label>
                            )} />
                            <Controller control={control} name="track_batch" render={({ field }) => (
                                <label className="flex items-center gap-2 cursor-pointer rounded-lg border border-default p-3 hover:bg-surface-50 transition-colors">
                                    <input type="checkbox" checked={field.value} onChange={e => field.onChange(e.target.checked)}
                                        className="h-4 w-4 rounded border-default text-brand-600 focus:ring-brand-500" />
                                    <div>
                                        <span className="text-sm font-medium text-surface-700">Controlar por lote</span>
                                        <p className="text-xs text-surface-400">Rastreia validade e lote</p>
                                    </div>
                                </label>
                            )} />
                            <Controller control={control} name="track_serial" render={({ field }) => (
                                <label className="flex items-center gap-2 cursor-pointer rounded-lg border border-default p-3 hover:bg-surface-50 transition-colors">
                                    <input type="checkbox" checked={field.value} onChange={e => field.onChange(e.target.checked)}
                                        className="h-4 w-4 rounded border-default text-brand-600 focus:ring-brand-500" />
                                    <div>
                                        <span className="text-sm font-medium text-surface-700">Controlar por nº série</span>
                                        <p className="text-xs text-surface-400">Cada unidade tem número único</p>
                                    </div>
                                </label>
                            )} />
                            <Controller control={control} name="is_kit" render={({ field }) => (
                                <label className="flex items-center gap-2 cursor-pointer rounded-lg border border-default p-3 hover:bg-surface-50 transition-colors">
                                    <input type="checkbox" checked={field.value} onChange={e => field.onChange(e.target.checked)}
                                        className="h-4 w-4 rounded border-default text-brand-600 focus:ring-brand-500" />
                                    <div>
                                        <span className="text-sm font-medium text-surface-700">É um kit</span>
                                        <p className="text-xs text-surface-400">Conjunto de outros produtos</p>
                                    </div>
                                </label>
                            )} />
                        </div>
                    </fieldset>

                    <hr className="border-subtle" />

                    {/* ── Fiscal ── */}
                    <fieldset className="space-y-4">
                        <legend className="text-sm font-semibold text-surface-700 uppercase tracking-wide mb-2">Fiscal</legend>
                        <div className="grid gap-4 sm:grid-cols-2">
                            <FormField label="NCM" error={errors.ncm?.message}>
                                <Input {...register('ncm')} placeholder="Ex: 84189900" maxLength={10} />
                            </FormField>
                            <div />
                        </div>
                    </fieldset>

                    <hr className="border-subtle" />

                    {/* ── Identificação Complementar ── */}
                    <fieldset className="space-y-4">
                        <legend className="text-sm font-semibold text-surface-700 uppercase tracking-wide mb-2">Identificação Complementar</legend>
                        <div className="grid gap-4 sm:grid-cols-2">
                            <FormField label="Código de Barras (EAN/GTIN)" error={errors.barcode?.message}>
                                <Input {...register('barcode')} placeholder="Ex: 7891234567890" maxLength={50} />
                            </FormField>
                            <FormField label="URL da Foto" error={errors.image_url?.message}>
                                <Input {...register('image_url')} placeholder="https://..." type="url" />
                            </FormField>
                        </div>
                    </fieldset>

                    <hr className="border-subtle" />

                    {/* ── Informações Físicas ── */}
                    <fieldset className="space-y-4">
                        <legend className="text-sm font-semibold text-surface-700 uppercase tracking-wide mb-2">Informações Físicas</legend>
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            <FormField label="Peso (kg)" error={errors.weight?.message}>
                                <Input {...register('weight')} type="number" step="0.001" placeholder="0.000" />
                            </FormField>
                            <FormField label="Largura (cm)" error={errors.width?.message}>
                                <Input {...register('width')} type="number" step="0.01" placeholder="0.00" />
                            </FormField>
                            <FormField label="Altura (cm)" error={errors.height?.message}>
                                <Input {...register('height')} type="number" step="0.01" placeholder="0.00" />
                            </FormField>
                            <FormField label="Profundidade (cm)" error={errors.depth?.message}>
                                <Input {...register('depth')} type="number" step="0.01" placeholder="0.00" />
                            </FormField>
                        </div>
                    </fieldset>

                    <hr className="border-subtle" />

                    {/* ── Status e Conexões ── */}
                    <fieldset className="space-y-3">
                        <Controller control={control} name="is_active" render={({ field }) => (
                            <label className="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" checked={field.value} onChange={e => field.onChange(e.target.checked)}
                                    className="h-4 w-4 rounded border-default text-brand-600 focus:ring-brand-500" />
                                <span className="text-sm font-medium text-surface-700">Produto ativo</span>
                            </label>
                        )} />
                        {editing && productEquipmentModels.length > 0 && (
                            <div>
                                <label className="mb-1.5 block text-sm font-medium text-surface-700">Usado nos modelos de balança</label>
                                <ul className="rounded-lg border border-default bg-surface-50 px-3.5 py-2.5 text-sm space-y-1">
                                    {(productEquipmentModels || []).map((m) => (
                                        <li key={m.id} className="flex items-center gap-2">
                                            <Scale className="h-3.5 w-3.5 text-surface-400 shrink-0" />
                                            {m.brand ? `${m.brand} - ${m.name}` : m.name}
                                        </li>
                                    ))}
                                </ul>
                                <p className="mt-1 text-xs text-surface-500">Para alterar, edite em Equipamentos → Modelos de balança.</p>
                            </div>
                        )}
                    </fieldset>

                    <div className="flex items-center justify-end gap-3 border-t border-subtle pt-4">
                        <Button variant="outline" type="button" onClick={() => setShowForm(false)}>Cancelar</Button>
                        <Button type="submit" loading={saveMut.isPending}>{editing ? 'Salvar' : 'Criar'}</Button>
                    </div>
                </form>
            </Modal>

            <Modal open={!!showConfirmDelete} onOpenChange={() => setShowConfirmDelete(null)} size="sm" title="Excluir Produto">
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-red-100 flex-shrink-0">
                            <AlertTriangle className="h-5 w-5 text-red-600" />
                        </div>
                        <div>
                            <h3 className="font-medium text-surface-900">Tem certeza?</h3>
                            <p className="text-sm text-surface-500">
                                Deseja realmente excluir <strong>{showConfirmDelete?.name}</strong>?
                            </p>
                        </div>
                    </div>

                    {deleteMessage && (
                        <div className="rounded-lg bg-red-50 p-3 text-sm text-red-700 border border-red-100">
                            <p className="font-medium mb-1">Não é possível excluir:</p>
                            <p>{deleteMessage}</p>
                        </div>
                    )}

                    {deleteDependencies && (
                        <div className="space-y-2">
                            <p className="text-xs font-medium text-surface-600 uppercase tracking-wide">Vínculos encontrados:</p>
                            <div className="grid grid-cols-2 gap-2">
                                {Object.entries(deleteDependencies).map(([key, count]) => (
                                    <div key={key} className="flex items-center justify-between rounded bg-surface-50 px-3 py-2 text-sm border border-default">
                                        <span className="text-surface-600 capitalize">{key.replace(/_/g, ' ')}</span>
                                        <Badge variant="neutral">{String(count)}</Badge>
                                    </div>
                                ))}
                            </div>
                        </div>
                    )}

                    <div className="flex justify-end gap-2 pt-2">
                        <Button variant="outline" onClick={() => setShowConfirmDelete(null)}>Cancelar</Button>
                        {deleteDependencies ? (
                            <Button variant="ghost" disabled className="text-surface-400 cursor-not-allowed">
                                Resolva as pendências acima
                            </Button>
                        ) : (
                            <Button className="bg-red-600 hover:bg-red-700 text-white" loading={deleteMut.isPending}
                                onClick={() => {
                                    if (showConfirmDelete) {
                                        deleteMut.mutate(showConfirmDelete.id)
                                    }
                                }}>
                                Excluir Produto
                            </Button>
                        )}
                    </div>
                </div>
            </Modal>
        </div>
    )
}
